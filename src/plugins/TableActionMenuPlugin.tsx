...
function TableActionMenu({
  ...
  useEffect(() => {
    // listens for when a lexical node is mutated
    // three mutatation states: created, destroyed, updated
    return editor.registerMutationListener(TableCellNode, (nodeMutations) => {
      ...
      if (nodeUpdated) {
        editor.getEditorState().read(() => {
          // Why does this need to be called in editor.getEditorState().read()?
  ...
  const insertTableRowAtSelection = useCallback(
    (shouldInsertAfter: boolean) => {
      editor.update(() => {
        ...
        $insertTableRow(
          tableNode, // The table the row is inserted into
          tableRowIndex, // Where the row row is inserted
          shouldInsertAfter, // boolean flag, insert above or below row
          selectionCounts.rows, // how many rows to insert
          grid, // not sure what this is yet
        )
  ...
  return createPortal(
    // React create Portal - allows rendering jsx elsewhere in the DOM
    // Could be replaced with a MUI menu component? Should it be?
    <div
      className="dropdown"
      ref={dropDownRef}
      onClick={(e) => {
...
function TableCellActionMenuContainer({
  ...
  useEffect(() => {
    // anchorElem is just used to adjust the position of the menu button
